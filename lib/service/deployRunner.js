...
/**
 * Validate that a repo URL is an HTTPS GitHub or GitLab URL.
 * SSH remote URLs work transparently when the server has an SSH key
 * configured - those do not need to be validated here because `git clone`
 * will refuse invalid addresses on its own.
 *
 * @param {string} url
 * @returns {boolean}
 */
...
/**
 * Parse a .env file relative to the deploy path and return key-value pairs.
 ...
 * @param {string} relPath - Relative path of the .env file within the repo.
 * @returns {object} Key-value map of env vars, or {} on error.
 */
export function parseEnvFile(deployPath, relPath) {
  if (!relPath) return {};
  let content;
  try {
    // Synchronous read is fine here - this file is always small.
    content = fs.readFileSync(path.join(deployPath, relPath), 'utf8');
  } catch {
    return {};
  }
  const result = {};
...
export async function runDeploy(deployment, { isRedeploy }) {
  ...
  try {
    ...
    if (isRedeploy) {
      try {
        await pm2.restartProcess(pm2_name);
      } catch {
        // Process may have been deleted from PM2 manually - fall back to start.
